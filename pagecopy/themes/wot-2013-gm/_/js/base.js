...
WideOpenTech = function(){
    ...
    app.headerAnimation = new HeaderAnimation();
...
HeaderAnimation = {
    ...
    events: function() {
        //adds class to change background of header on scroll
    ...
    checkPosition: function(){
        if ($(window).scrollTop() > 10) {
            HeaderAnimation.deactivateTransparentNav();
        } else {
            HeaderAnimation.activateTransparentNav();
        }
    },
    activateTransparentNav: function(){
        //remove the background property so it comes transparent again
        $("header").removeClass("shrink");
    },
    deactivateTransparentNav: function(){
...
NewsletterSignup = {
    ...
    events: function(){
        $(window).scroll(function() {
            if ($(document).scrollTop() > 768 && $("#newsletter-signup").attr("displayed") === "false" && !NewsletterSignup.hasCookie()) {
                $('#newsletter-signup').modal('show');
                $("#newsletter-signup").attr("displayed", "true");
            }
        });

        //Set cookie when user closes modal
        $('#newsletter-signup').on('hidden.bs.modal', function (e) {
            NewsletterSignup.setCookie();
        })
    },
    setCookie: function(){
        $.cookie("wot_newsletter", 1, { expires : 10 });
    },
    hasCookie: function(){
        if ($.cookie('wot_newsletter')) {
            return true;
        } else {
            return false;
        }
...
ConnectWave = {
    ...
    events: function() {
// make some waves.
        var ocean = document.getElementById("connect-wave"),
            waveWidth = 3,
            waveCount = Math.floor((window.innerWidth / waveWidth) + 150),
        ...
        for (var i = 0; i < waveCount; i++) {
            ...
            wave.style.webkitAnimationDelay = (i/250) + "s";
        }
